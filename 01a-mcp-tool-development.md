...
[<- Back to Main Topic](./01-model-context-procol_mcp.md) | [Next Sub-Topic: Server Implementation ->](./02-mcp-server-implementation.md)
...
const atomicTools = {
  createNote: async (filename, content) => {
    /* single responsibility */
  },
  readNote: async (filename) => {
    /* single responsibility */
  },
  updateNote: async (filename, content) => {
    /* single responsibility */
  },
    ...
    for (const file of files) {
      try {
        const content = await fs.readFile(
          path.join(this.notesDirectory, file),
          "utf8"
        );
